...
# Embeddings: Chroma's built-in default (local ONNX MiniLM) — no external API needed.
# Note: switching embedding models requires re-uploading previously indexed documents.
vectorstore = Chroma(
...
def add_document_to_rag(file_path: str, thread_id: str):
    ...
    # Re-uploading a same-named file replaces its previous chunks instead of stacking duplicates.
